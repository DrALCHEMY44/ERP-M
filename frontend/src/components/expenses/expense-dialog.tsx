...
export function ExpenseDialog({ expense, open, onOpenChange, onSave }: ExpenseDialogProps) {
  ...
  const onSubmit = async (values: ExpenseFormValues) => {
    ...
    try {
      ...
      // The parent displays the database error toast; keep the form open so the
      // user can correct or retry the entry without losing their values.
    } finally {
      setIsSaving(false)
    }
  }
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" className="bg-amber-600 hover:bg-amber-700 text-white" disabled={isSaving}>
                {isSaving ? "Saving…" : expense ? "Update Record" : "Save Expense"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
