...
interface DocumentDialogProps {
  ...
  onSave: (doc: Partial<BusinessDocument>) => Promise<string | void>
}
...
export function DocumentDialog({ open, onOpenChange, onSave }: DocumentDialogProps) {
  ...
  const onSubmit = async (values: DocumentFormValues) => {
    ...
    if (!user || !profile?.tenantId || !profile.businessId) {
      form.setError("root", { message: "You must be signed in with a business profile to upload files." })
      return
    }
    ...
    try {
      ...
      const documentId = await onSave({
        ...values,
        tenantId: profile.tenantId,
        businessId: profile.businessId,
        uploadedBy: user.uid,
        uploadedByName: profile.fullName || user.displayName || user.email || "Member",
