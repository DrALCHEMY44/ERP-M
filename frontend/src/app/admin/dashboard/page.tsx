  ...
  return (
    <article className="rounded-[22px] border border-white bg-white p-5 shadow-[0_12px_35px_rgba(51,65,85,0.07)]">
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-slate-400">{label}</p><p className="mt-3 text-2xl font-black tracking-tight text-[#17223b]">{value}</p><p className="mt-1 text-[10px] text-slate-400">{detail}</p></div>
...
function Empty({ icon: Icon, title, copy }: { icon: React.ComponentType<{ className?: string }>; title: string; copy: string }) {
  return <div className="flex min-h-40 flex-col items-center justify-center p-8 text-center"><Icon className="size-8 text-slate-200" /><p className="mt-3 text-sm font-bold text-slate-600">{title}</p><p className="mt-1 text-xs text-slate-400">{copy}</p></div>
}
...
export default function SuperAdminDashboard() {
  ...
  if (authLoading || loading && !overview) return <div className="grid min-h-[75vh] place-items-center bg-[#f6f7fb]"><Loader2 className="size-8 animate-spin text-cyan-500" /></div>
  if (!overview) return <div className="grid min-h-[75vh] place-items-center bg-[#f6f7fb] p-6"><div className="max-w-md rounded-3xl bg-white p-8 text-center shadow-xl"><XCircle className="mx-auto size-10 text-rose-500" /><h1 className="mt-4 text-lg font-black text-slate-800">SaaS control plane unavailable</h1><p className="mt-2 text-sm text-slate-500">{error?.message || "The platform overview could not be loaded."}</p><Button onClick={() => void refetchOverview()} className="mt-5">Try again</Button></div></div>
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fb] text-slate-700">
      <header className="border-b border-slate-200/70 bg-white/90 backdrop-blur"><div className="mx-auto flex max-w-[1600px] flex-col gap-4 px-4 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-600 text-white shadow-lg shadow-cyan-500/20"><ShieldCheck className="size-6" /></span><div><p className="text-[9px] font-black uppercase tracking-[.22em] text-cyan-600">Platform administration</p><h1 className="text-xl font-black tracking-tight text-[#17223b]">SmartERP SaaS Control Plane</h1><p className="text-[10px] text-slate-400">Signed in as {profile?.email || "Platform Super Admin"} · refreshed {dateTime(overview.generatedAt)}</p></div></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline" className="rounded-xl"><Link href="/admin/users"><Users className="mr-2 size-4" />Manage users</Link></Button><Button variant="outline" onClick={() => void refresh()} disabled={refreshing} className="rounded-xl"><RefreshCw className={`mr-2 size-4 ${refreshing ? "animate-spin" : ""}`} />Refresh</Button><Button variant="outline" onClick={() => void exportTenants()} disabled={pending === "export"} className="rounded-xl"><Download className="mr-2 size-4" />Export</Button><Button onClick={() => setCreateOpen(true)} className="rounded-xl bg-cyan-600 hover:bg-cyan-700"><Plus className="mr-2 size-4" />New workspace</Button></div></div></header>
      ...
      <main className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6 lg:p-8">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Actual MRR" value={money(overview.totals.mrrFcfa)} detail={`${paidConversion}% paid conversion`} icon={CircleDollarSign} tone="bg-cyan-50 text-cyan-600" />
          <MetricCard label="Collected · 30 days" value={money(overview.totals.collected30dFcfa)} detail={`${money(overview.totals.outstandingFcfa)} outstanding`} icon={TrendingUp} tone="bg-emerald-50 text-emerald-600" />
          <MetricCard label="Workspaces" value={compact(overview.totals.tenants)} detail={`${overview.totals.activeTenants} active · ${overview.totals.suspendedTenants} suspended`} icon={Building2} tone="bg-violet-50 text-violet-600" />
          <MetricCard label="Active users · 30 days" value={compact(overview.totals.activeUsers30d)} detail={`${overview.totals.users} registered platform users`} icon={Users} tone="bg-amber-50 text-amber-600" />
        </section>

        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl border border-white bg-white p-1.5 shadow-sm"><TabsTrigger value="overview" className="rounded-xl text-xs"><BarChart3 className="mr-2 size-4" />Overview</TabsTrigger><TabsTrigger value="workspaces" className="rounded-xl text-xs"><Building2 className="mr-2 size-4" />Workspaces</TabsTrigger><TabsTrigger value="users" className="rounded-xl text-xs"><UserCog className="mr-2 size-4" />Users</TabsTrigger><TabsTrigger value="billing" className="rounded-xl text-xs"><CreditCard className="mr-2 size-4" />Billing</TabsTrigger><TabsTrigger value="support" className="rounded-xl text-xs"><Headphones className="mr-2 size-4" />Support</TabsTrigger><TabsTrigger value="audit" className="rounded-xl text-xs"><ShieldCheck className="mr-2 size-4" />Audit</TabsTrigger></TabsList>

          <TabsContent value="overview" className="mt-5 space-y-5">
            <section className="grid gap-5 xl:grid-cols-[1.6fr_1fr]">
              <article className="rounded-[22px] border border-white bg-white p-5 shadow-[0_12px_35px_rgba(51,65,85,0.07)]"><div className="flex items-start justify-between"><div><h2 className="text-sm font-extrabold text-[#17223b]">Collected revenue</h2><p className="mt-1 text-[10px] text-slate-400">Recorded SaaS payments and workspace growth</p></div><Badge variant="outline">Last 6 months</Badge></div><div className="mt-5 h-72"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={overview.trend} margin={{ left: 0, right: 8, top: 10, bottom: 0 }}><defs><linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#06b6d4" stopOpacity={0.32} /><stop offset="100%" stopColor="#06b6d4" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid stroke="#eef2f7" vertical={false} /><XAxis dataKey="month" tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} /><YAxis yAxisId="money" tickFormatter={compact} tick={{ fontSize: 9, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={42} /><YAxis yAxisId="workspaces" orientation="right" tick={{ fontSize: 9, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={28} /><Tooltip formatter={(value, name) => name === "collectedFcfa" ? [money(Number(value)), "Collected"] : [value, "Workspaces"]} contentStyle={{ borderRadius: 14, border: "1px solid #e2e8f0", fontSize: 11 }} /><Area yAxisId="money" type="monotone" dataKey="collectedFcfa" stroke="#06b6d4" fill="url(#revenueFill)" strokeWidth={2.5} /><Bar yAxisId="workspaces" dataKey="workspaces" fill="#8b5cf6" opacity={0.65} radius={[4, 4, 0, 0]} barSize={13} /></ComposedChart></ResponsiveContainer></div></article>
              <article className="rounded-[22px] border border-white bg-white p-5 shadow-[0_12px_35px_rgba(51,65,85,0.07)]"><h2 className="text-sm font-extrabold text-[#17223b]">Platform operations</h2><p className="mt-1 text-[10px] text-slate-400">Current service and workload signals</p><div className="mt-5 space-y-3">{[
          ...
          <TabsContent value="users" className="mt-5"><article className="overflow-hidden rounded-[22px] border border-white bg-white shadow-[0_12px_35px_rgba(51,65,85,0.07)]"><div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-extrabold text-[#17223b]">Platform user administration</h2><p className="mt-1 text-[10px] text-slate-400">Roles, account access, sessions, login activity, and password recovery</p></div><div className="relative"><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-slate-300" /><Input value={userSearch} onChange={(event) => setUserSearch(event.target.value)} placeholder="Search users…" className="h-9 rounded-xl pl-9 text-xs sm:w-72" /></div></div><div className="overflow-x-auto"><Table><TableHeader><TableRow className="bg-slate-50/60"><TableHead>User</TableHead><TableHead>Workspace</TableHead><TableHead>Role</TableHead><TableHead>Last login</TableHead><TableHead>Sessions</TableHead><TableHead>Status</TableHead><TableHead /></TableRow></TableHeader><TableBody>{filteredUsers.map((user) => <TableRow key={user.id}><TableCell><p className="font-bold text-slate-700">{user.fullName || "Unnamed user"}</p><p className="text-[10px] text-slate-400">{user.email} · {user.authLinked ? "Auth linked" : "Invite pending"}</p></TableCell><TableCell className="text-xs text-slate-600">{user.tenantName}</TableCell><TableCell><Badge variant="outline">{user.role}</Badge></TableCell><TableCell className="text-[10px] text-slate-500">{dateTime(user.lastLoginAt)}</TableCell><TableCell className="text-xs font-bold">{user.activeSessions}</TableCell><TableCell><StatusBadge status={user.status} /></TableCell><TableCell><UserActions user={user} pending={pending?.includes(user.id) ?? false} run={run} sendReset={sendReset} /></TableCell></TableRow>)}{!filteredUsers.length && <TableRow><TableCell colSpan={7}><Empty icon={Users} title="No users found" copy="Try a different user search." /></TableCell></TableRow>}</TableBody></Table></div></article></TabsContent>
...
function SupportTable({ cases, pending, run }: { cases: PlatformSupportCase[]; pending: string | null; run: RunAction }) {
  return <article className="overflow-hidden rounded-[22px] border border-white bg-white shadow-[0_12px_35px_rgba(51,65,85,0.07)]"><div className="border-b border-slate-100 p-4"><h2 className="text-sm font-extrabold text-[#17223b]">Support queue</h2><p className="mt-1 text-[10px] text-slate-400">Internal platform cases, priority, ownership, and resolution status</p></div><div className="overflow-x-auto"><Table><TableHeader><TableRow className="bg-slate-50/60"><TableHead>Case</TableHead><TableHead>Workspace</TableHead><TableHead>Priority</TableHead><TableHead>Assigned</TableHead><TableHead>Updated</TableHead><TableHead>Status</TableHead><TableHead /></TableRow></TableHeader><TableBody>{cases.map((supportCase) => <TableRow key={supportCase.id}><TableCell><p className="font-bold text-slate-700">{supportCase.subject}</p><p className="max-w-72 truncate text-[10px] text-slate-400">{supportCase.description || "No description"}</p></TableCell><TableCell className="text-xs">{supportCase.tenantName}</TableCell><TableCell><StatusBadge status={supportCase.priority} /></TableCell><TableCell className="text-[10px]">{supportCase.assignedTo || "Unassigned"}</TableCell><TableCell className="text-[10px]">{dateTime(supportCase.updatedAt)}</TableCell><TableCell><StatusBadge status={supportCase.status} /></TableCell><TableCell><SupportActions supportCase={supportCase} pending={pending?.includes(supportCase.id) ?? false} run={run} /></TableCell></TableRow>)}{!cases.length && <TableRow><TableCell colSpan={7}><Empty icon={Headphones} title="Support queue is clear" copy="New internal cases will appear here." /></TableCell></TableRow>}</TableBody></Table></div></article>
}
...
function AuditTable({ entries }: { entries: PlatformOverview["audit"] }) {
  ...
  return <article className="overflow-hidden rounded-[22px] border border-white bg-white shadow-[0_12px_35px_rgba(51,65,85,0.07)]"><div className="flex items-center justify-between border-b border-slate-100 p-4"><div><h2 className="text-sm font-extrabold text-[#17223b]">Platform audit trail</h2><p className="mt-1 text-[10px] text-slate-400">Immutable administrative action history with actor and change details</p></div><Button variant="outline" size="sm" onClick={exportAudit}><Download className="mr-2 size-3.5" />Export</Button></div><div className="overflow-x-auto"><Table><TableHeader><TableRow className="bg-slate-50/60"><TableHead>Time</TableHead><TableHead>Actor</TableHead><TableHead>Action</TableHead><TableHead>Target</TableHead><TableHead>Details</TableHead></TableRow></TableHeader><TableBody>{entries.map((entry) => <TableRow key={entry.id}><TableCell className="whitespace-nowrap text-[10px]">{dateTime(entry.createdAt)}</TableCell><TableCell className="text-xs">{entry.actorEmail}</TableCell><TableCell><Badge variant="outline">{entry.action}</Badge></TableCell><TableCell><p className="text-xs font-bold">{entry.targetType}</p><p className="max-w-40 truncate text-[9px] text-slate-400">{entry.targetId}</p></TableCell><TableCell className="max-w-sm"><p className="truncate text-[10px] text-slate-500">{JSON.stringify(entry.details)}</p></TableCell></TableRow>)}{!entries.length && <TableRow><TableCell colSpan={5}><Empty icon={ShieldCheck} title="No platform changes yet" copy="Administrative actions will be recorded here." /></TableCell></TableRow>}</TableBody></Table></div></article>
}
...
function TenantDetailsDialog({ details, loading, open, onOpenChange, pending, run, sendReset, changeStatus }: { details: PlatformTenantDetails | null; loading: boolean; open: boolean; onOpenChange: (open: boolean) => void; pending: string | null; run: RunAction; sendReset: (user: PlatformUser) => void; changeStatus: (tenant: PlatformTenant) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto p-0"><DialogHeader className="border-b border-slate-100 p-6 pb-4">{details ? <><div className="flex flex-wrap items-center gap-2"><DialogTitle>{details.tenant.name}</DialogTitle><StatusBadge status={details.tenant.status} /><Badge className={`border-0 ring-1 ${planTone[details.tenant.plan]}`}>{details.tenant.plan}</Badge></div><DialogDescription>{details.tenant.ownerEmail} · {details.tenant.businessSector} · {details.tenant.location}</DialogDescription></> : <><DialogTitle>Workspace control record</DialogTitle><DialogDescription>Loading tenant configuration and activity.</DialogDescription></>}</DialogHeader>{loading && !details ? <div className="space-y-3 p-6">{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-14" />)}</div> : details ? <div className="p-6"><div className="grid gap-3 sm:grid-cols-4">{[["Users", `${details.tenant.userCount} / ${details.plan.maxUsers ?? "∞"}`], ["Businesses", `${details.tenant.businessCount} / ${details.plan.maxBusinesses ?? "∞"}`], ["Documents", `${details.tenant.documentCount} / ${details.plan.maxDocuments ?? "∞"}`], ["AI this month", `${details.tenant.aiRequestsThisMonth} / ${details.plan.monthlyAiRequests ?? "∞"}`]].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-3"><p className="text-[9px] font-black uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 text-lg font-black text-slate-700">{value}</p></div>)}</div><Tabs defaultValue="subscription" className="mt-5"><TabsList className="h-auto w-full justify-start overflow-x-auto"><TabsTrigger value="subscription">Subscription</TabsTrigger><TabsTrigger value="members">Members</TabsTrigger><TabsTrigger value="invoices">Invoices</TabsTrigger><TabsTrigger value="notes">Notes</TabsTrigger><TabsTrigger value="support">Support</TabsTrigger><TabsTrigger value="audit">Audit</TabsTrigger></TabsList>
    ...
    <TabsContent value="notes" className="space-y-3 pt-3"><div className="flex gap-2"><Textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add an internal tenant note…" /><Button disabled={!note.trim() || pending === `note:${details.tenant.id}`} onClick={() => { const body = note; setNote(""); run(`note:${details.tenant.id}`, () => addPlatformTenantNote(details.tenant.id, body), "Tenant note added") }}>Add note</Button></div>{details.notes.map((item) => <div key={item.id} className="rounded-2xl border p-4"><p className="whitespace-pre-wrap text-sm text-slate-600">{item.body}</p><p className="mt-2 text-[9px] text-slate-400">{item.createdByEmail} · {dateTime(item.createdAt)}</p></div>)}{!details.notes.length && <Empty icon={FileText} title="No internal notes" copy="Record onboarding, support, or account context here." />}</TabsContent>
    <TabsContent value="support" className="space-y-2 pt-3">{details.supportCases.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl border p-3"><Headphones className="size-4 text-violet-500" /><div className="flex-1"><p className="text-xs font-bold">{item.subject}</p><p className="text-[9px] text-slate-400">{item.assignedTo || "Unassigned"} · {dateTime(item.updatedAt)}</p></div><StatusBadge status={item.priority} /><StatusBadge status={item.status} /></div>)}{!details.supportCases.length && <Empty icon={Headphones} title="No support cases" copy="Open one from the Support tab." />}</TabsContent>
    <TabsContent value="audit" className="space-y-2 pt-3">{details.audit.map((item) => <div key={item.id} className="flex items-start gap-3 rounded-2xl border p-3"><ShieldCheck className="mt-0.5 size-4 text-cyan-500" /><div><p className="text-xs font-bold">{item.action}</p><p className="text-[10px] text-slate-500">{item.actorEmail} · {dateTime(item.createdAt)}</p><p className="mt-1 text-[9px] text-slate-400">{JSON.stringify(item.details)}</p></div></div>)}{!details.audit.length && <Empty icon={ShieldCheck} title="No audit entries" copy="Future administrative changes will appear here." />}</TabsContent>
