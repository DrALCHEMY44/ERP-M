  ...
  return (
    <Dialog open={Boolean(user)} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-4 py-2 sm:grid-cols-2">
          <div className="space-y-1.5"><Label>Full name</Label><Input value={form.fullName} onChange={(event) => setForm((value) => ({ ...value, fullName: event.target.value }))} /></div>
          <div className="space-y-1.5"><Label>Email</Label><Input type="email" value={form.email} disabled={Boolean(user?.authLinked)} onChange={(event) => setForm((value) => ({ ...value, email: event.target.value }))} />{user?.authLinked && <p className="text-xs text-slate-500">Managed by the authentication account.</p>}</div>
          ...
          <div className="space-y-1.5"><Label>Role</Label><Select value={form.role} onValueChange={(role) => setForm((value) => ({ ...value, role }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{roles.filter((role) => role !== "Business Owner" || user?.authLinked).map((role) => <SelectItem key={role} value={role}>{role}</SelectItem>)}</SelectContent></Select>{!user?.authLinked && <p className="text-xs text-slate-500">Ownership requires completed registration.</p>}</div>
        ...
        {loading && !details ? <div className="space-y-3 p-6">{Array.from({ length: 5 }).map((_, index) => <Skeleton className="h-14" key={index} />)}</div> : details ? (
          <div className="p-6">
            <div className="grid gap-3 sm:grid-cols-4">
              {[["Role", details.user.role], ["Department", details.user.department || "Not set"], ["Last login", dateTime(details.user.lastLoginAt)], ["Active sessions", String(details.user.activeSessions)]].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 truncate text-xs font-bold text-slate-700" title={value}>{value}</p></div>)}
            </div>
            <div className="mt-4 flex flex-wrap items-center gap-2"><StatusBadge status={details.user.authLinked ? "linked" : details.user.invitationStatus || "not invited"} /><span className="text-xs text-slate-500">Created {dateTime(details.user.createdAt)}</span><Button variant="outline" size="sm" className="ml-auto" onClick={() => onEdit(details.user)}><Pencil className="mr-2 size-3.5" />Edit user</Button></div>
            <Tabs defaultValue="activity" className="mt-5">
              ...
              <TabsContent value="activity" className="space-y-2 pt-3">{details.activity.map((entry) => <div key={entry.id} className="flex gap-3 rounded-2xl border p-3"><Activity className="mt-0.5 size-4 text-primary" /><div><p className="text-xs font-bold">{entry.actionType} · {entry.module}</p><p className="text-xs text-slate-500">{entry.description || "No description"}</p><p className="mt-1 text-xs text-slate-500">{dateTime(entry.timestamp)}</p></div></div>)}{!details.activity.length && <p className="rounded-2xl border border-dashed p-8 text-center text-xs text-slate-500">No recorded ERP activity.</p>}</TabsContent>
              <TabsContent value="sessions" className="space-y-2 pt-3">{details.sessions.map((session) => <div key={session.id} className="flex items-start gap-3 rounded-2xl border p-3"><KeyRound className="mt-0.5 size-4 text-violet-500" /><div className="min-w-0 flex-1"><p className="text-xs font-bold">Last used {dateTime(session.lastUsedAt)}</p><p className="truncate text-xs text-slate-500">{session.userAgent || "Unknown device"}</p><p className="mt-1 text-xs text-slate-500">Created {dateTime(session.createdAt)} · expires {dateTime(session.expiresAt)}</p></div><StatusBadge status={session.revokedAt ? "revoked" : new Date(session.expiresAt) <= new Date() ? "expired" : "Active"} /></div>)}{!details.sessions.length && <p className="rounded-2xl border border-dashed p-8 text-center text-xs text-slate-500">No employee access-code sessions.</p>}</TabsContent>
              <TabsContent value="audit" className="space-y-2 pt-3">{details.audit.map((entry) => <div key={entry.id} className="flex gap-3 rounded-2xl border p-3"><ShieldCheck className="mt-0.5 size-4 text-emerald-500" /><div><p className="text-xs font-bold">{entry.action}</p><p className="text-xs text-slate-500">{entry.actorEmail} · {dateTime(entry.createdAt)}</p><p className="mt-1 max-w-2xl truncate text-xs text-slate-500">{JSON.stringify(entry.details)}</p></div></div>)}{!details.audit.length && <p className="rounded-2xl border border-dashed p-8 text-center text-xs text-slate-500">No administrative changes for this user.</p>}</TabsContent>
            </Tabs>
          </div>
        ) : null}
      </DialogContent>
...
export default function PlatformUsersPage() {
  ...
  if (authLoading || (loading && !directory)) return <div className="min-h-screen bg-background p-6"><div className="mx-auto max-w-[1440px] space-y-4">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-24 rounded-xl" />)}</div></div>
  if (profile?.role !== "Platform Super Admin") return <div className="grid min-h-[60vh] place-items-center p-6 text-center"><div><Ban className="mx-auto size-10 text-rose-500" /><h1 className="mt-3 text-xl font-semibold">Platform access required</h1><p className="mt-1 text-sm text-slate-500">Only Platform Super Admins can manage all platform users.</p></div></div>
  if (error || !directory) return <div className="grid min-h-[60vh] place-items-center p-6 text-center"><div><XCircle className="mx-auto size-10 text-rose-500" /><h1 className="mt-3 text-xl font-semibold">Could not load platform users</h1><p className="mt-1 text-sm text-slate-500">{error?.message || "Please retry."}</p><Button className="mt-4" onClick={refresh}>Retry</Button></div></div>

  const pageCount = Math.max(1, Math.ceil(directory.total / directory.pageSize))
  return (
    <div className="text-slate-900">
      <div className="mx-auto max-w-[1440px] space-y-6">
        <header className="flex flex-col justify-between gap-4 border-b pb-6 xl:flex-row xl:items-center">
          <div>
            ...
            <div className="flex items-center gap-3"><div><h1 className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">Platform users</h1><p className="text-xs text-slate-500">Identity, access, invitations, roles, sessions, and user activity across every workspace</p></div></div>
          </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <MetricCard label="All users" value={compact(directory.totals.users)} detail="Across every workspace" icon={Users} />
          <MetricCard label="Active access" value={compact(directory.totals.active)} detail="Allowed to authenticate" icon={UserCheck} />
          <MetricCard label="Active 30d" value={compact(directory.totals.active30d)} detail="Logged in recently" icon={Activity} />
          <MetricCard label="Pending invites" value={compact(directory.totals.pendingInvitations)} detail="Valid registration links" icon={MailPlus} />
          <MetricCard label="Suspended" value={compact(directory.totals.suspended)} detail="Access currently blocked" icon={Ban} />
          <MetricCard label="Super admins" value={compact(directory.totals.superAdmins)} detail="Active platform operators" icon={ShieldCheck} />
        </section>

        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-none">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            <div className="relative xl:col-span-2"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-500" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, workspace, business…" /></div>
